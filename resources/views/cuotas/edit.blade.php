        ...
        <form action="{{ route('cuotas.update', $cuota) }}" method="POST" id="formCuota">
                ...
                <div class="col-md-12">
                    ...
                    <div class="d-flex flex-wrap gap-3">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="alcance" id="alc_general" value="general" {{ old('alcance', $cuota->alcance ?? 'bloque') === 'general' ? 'checked' : '' }}>
                            <label class="form-check-label" for="alc_general">General (toda la escuela)</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="alcance" id="alc_sede" value="sede" {{ old('alcance', $cuota->alcance ?? 'bloque') === 'sede' ? 'checked' : '' }}>
                            <label class="form-check-label" for="alc_sede">Diferencial por sede</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="alcance" id="alc_bloque" value="bloque" {{ old('alcance', $cuota->alcance ?? 'bloque') === 'bloque' ? 'checked' : '' }}>
            ...
            <div class="row mb-3" id="rowSede" style="display: none;">
                <div class="col-md-12">
                    ...
                    <select name="sede_id" id="sede_id" class="form-select @error('sede_id') is-invalid @enderror">
                        ...
                        @foreach($sedes as $s)
                        <option value="{{ $s->id }}" {{ (string) old('sede_id', $cuota->sede_id) === (string) $s->id ? 'selected' : '' }}>{{ $s->nombre }}</option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3" id="rowBloque">
                <div class="col-md-12">
                    ...
                    <select name="bloque_id" id="bloque_id" class="form-select @error('bloque_id') is-invalid @enderror">
                        ...
                        <option value="{{ $b->id }}" {{ (string) old('bloque_id', $cuota->bloque_id) === (string) $b->id ? 'selected' : '' }} data-sede-id="{{ $b->sede_id ?? '' }}">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="nombre" class="form-control @error('nombre') is-invalid @enderror" value="{{ old('nombre', $cuota->nombre) }}" placeholder="Ej. Cuota Mayo 2026" required>
                ...
                <div class="col-md-3">
                    ...
                    <select name="mes" class="form-select @error('mes') is-invalid @enderror" required>
                        ...
                        <option value="{{ $n }}" {{ (string) old('mes', (string) ($cuota->mes ?? now()->month)) === (string) $n ? 'selected' : '' }}>{{ $nombre }}</option>
                        @endforeach
                    </select>
