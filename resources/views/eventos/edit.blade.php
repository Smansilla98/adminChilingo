...
>
    <x-slot:actions>
        <a href="{{ route('eventos.show', $evento) }}" class="btn btn-outline-secondary btn-sm">Ver</a>
        <a href="{{ route('eventos.index') }}" class="btn btn-outline-secondary btn-sm">Listado</a>
    </x-slot:actions>

    <form action="{{ route('eventos.update', $evento) }}" method="POST" class="ito-form">
        @csrf
        @method('PUT')
        <x-ito.form-steps :steps="['Qué', 'Cuándo y dónde']" submit-label="Actualizar">
            <x-slot:cancel>
