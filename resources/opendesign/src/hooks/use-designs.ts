...
export function useDesigns(
  ...
  getThumbnailForPage: (pageId: string) => string | null = () => null
) {
  ...
  const saveDesign = useCallback(async () => {
    ...
    try {
      ...
      const updated = await api<Design>("PUT", `/api/designs/${activeIdRef.current}`, {
        ...
        thumbnail_data: currentPages.length > 0 ? getThumbnailForPage(currentPages[0].id) : null,
      });
  ...
  }, [getCanvasJSONForPage, getThumbnailForPage, pages]);
