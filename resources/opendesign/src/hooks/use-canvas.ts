...
const TEXT_PRESETS = {
  heading: { text: "Agregá un título", fontSize: 48, fontWeight: "700", fontFamily: "Montserrat" },
  subheading: { text: "Agregá un subtítulo", fontSize: 32, fontWeight: "500", fontFamily: "Inter" },
  body: { text: "Agregá texto", fontSize: 18, fontWeight: "400", fontFamily: "Inter" },
} as const;
...
export function useCanvasState() {
  ...
  const exportPNG = useCallback(() => {
    ...
    link.download = "diseno-chilinga.png";
    link.href = dataURL;
    link.click();
  ...
  /** Miniatura JPEG (~480 px de ancho) para la galería de diseños. */
  const getThumbnailForPage = useCallback((pageId: string): string | null => {
    const canvas = canvasMapRef.current.get(pageId);
    if (!canvas) return null;
    try {
      const multiplier = Math.min(1, 480 / (canvas.getWidth() || 480));
      return canvas.toDataURL({ format: "jpeg", quality: 0.8, multiplier });
    } catch {
      return null; // imagen de otro origen: sin miniatura
    }
  }, []);
